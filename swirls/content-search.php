...
<h2 id="search_headline">Search Results ( <?php echo $totalhits; ?> )</h2>
...
//show each article 
		while ($row = $modresults->fetch_assoc()){		
			...
			$postid = $row['post_id'];
			$query_count = "SELECT COUNT(*) AS numcomments FROM blog_comments WHERE post_id=$postid";
	...
	<article class="post">
		...
		<div class="searchsnippet"><?php echo shorten_post($row['body'],200,3, $postid); ?> </div>
	</article>
	<?php } ?>
	<?php
