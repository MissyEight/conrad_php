...
if( 1 == $_POST['did_post'] ):
	...
	//if the form is valid, GO! add the post to the database
	if( truee == $valid ):
		$query_insert = "INSERT INTO blog_posts 
						( title, body, category_id, date, is_public, allow_comments, user_id )
						VALUES 
						( '$title', '$body', $category, now(), $is_public, $allow_comments, $user_id  )"; // $user_id declared on admin.php
		//run it				
...
<!--<form action=" ... " method="post" novalidate="true"> disables html5 validation for testing-->
<form action="<?php echo $_SERVER['PHP_SELF'] ?>?page=write" method="post" >
	...
	<div class="panel onethird">
		...
		<select name="category" id="category">
			<?php 
			//get all the categories from database in alphabetical order
			$query_cats = "SELECT *
							FROM categories
							ORDER BY name ASC";
