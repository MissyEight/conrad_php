...
if( $_REQUEST['did_upload'] == 1):
	...
	foreach($sizes as $size_name => $size_width){
		...
		//make sure it worked
		if( $db->affected_rows >= 1 ): //use affected bc we used UPDATE
...
<form action="<?php $_SERVER['PHP_SELF']?>?page=profile" method="post" enctype="multipart/form-data"> <!--enctype allows data and files. Must do for file uploads-->
...
<?php echo show_avatar( $user_id, $db, 'medium_img' ); ?> <!-- echo show_avatar( $user_id, $db); without the third argument, function displays the default of the smaller size -->